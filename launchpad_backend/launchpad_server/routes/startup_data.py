...
user_info = {
    ...
    "index": "userId",
    "records": [{
        "userId": user["records"][0].get("userId"),
        "email": user["records"][0].get("email"),
        "firstName": user["records"][0].get("firstName"),
    }]
}
...
startup_data = [application, user, posting, notification, company, user_info]
